<?php
	...
	if($action == 'ajax'){
		// escaping, additionally removing everything that could be (html/javascript-) code
		 ...
		 $aColumns = array('codigo_producto', 'nombre_producto','cantidad_producto');//Columnas de busqueda
		 $sTable = "tb_producto";
		 $sWhere = "";
		if ( $_GET['q'] != "" )
		{
			...
			<div class="table-responsive">
			  <table class="table">
				...
				while ($row=mysqli_fetch_array($query)){
					...
					//$status = ($stock == 0) ? " style='background-color: #be181857'" : "";
					?>
					<tr <?php //echo $status; ?> >
						...
						<!--td class='text-center'><?php if($stock!=0){  ?><a class='btn btn-info'href="#" onclick="agregar('<?php echo $id_producto ?>')"><i class="glyphicon glyphicon-plus"></i></a><?php } ?></td-->
					</tr>
					<?php
				}
				?>
