	...
	function agregar (id)
		...
		$(document).on('click','#imprimir_ticketera',function(e){
			id_factura = $('#id_factura').val();
			$.ajax({
					url:'./ajax/imprimirticketera.php?id_factura='+id_factura,
					 beforeSend: function(objeto){
					  //$('#loader').html('<img src="./img/ajax-loader.gif"> Cargando...');
				  },
					success:function(data){
		...
		$(document).on('click','#errorxml',function(e){
			Swal(
						      'Mensaje!',
						      "No se ha generado el xml",
						      "error"
